    ...
    <Card className="relative">
      ...
      <CardContent>
        <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex}>
          <Tab.List className="mb-4 flex space-x-1 rounded-xl bg-gray-700/20 p-1">
            <Tab 
              className={({ selected }) => 
                `w-full rounded-lg py-2 text-sm font-medium leading-5 
                 ${selected ? 'bg-primary-color text-white shadow' : 'text-text-secondary hover:bg-gray-700/30 hover:text-white'}`
              }
            >
              Распределение
            </Tab>
          ...
          <Tab.Panels className="mt-2">
            <Tab.Panel className="min-h-[220px]">
              {hasTokenomicsData ? (
